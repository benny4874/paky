...
class RecipeBuilderTest {
  ...
  private static final Instant INSTANT_IN_THE_PAST =
      LocalDate.of(2000, 1, 1).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant();
  ...
  public static Stream<Arguments> getInvalidRecipe() {
    ...
    return Stream.of(
        ...
        Arguments.of(
            pastExpirationDate, NoExpirationDateException.class, "Expiration date is invalid"),
        Arguments.of(
            labelNotValidJson, InvalidLabelException.class, "Label is in invalid json format"),
        Arguments.of(labelNull, InvalidLabelException.class, "Label is  null"),
        Arguments.of(
            negativeQuantity,
            InvalidQuantityException.class,
            "Quantity is invalid (must not be negative)"),
        Arguments.of(
            zeroQuantity,
            InvalidQuantityException.class,
            "Quantity is invalid (must be greater than 0)"));
  }

  @Test
  void build_assignId() {
    final Recipe result = RecipeBuilder.getFor("aVendor").with(aBaunchOfData).build();
  ...
  void build_throwsExceptionOnInvalidData(
      Consumer<RecipeBuilder> wrongData,
      Class<RuntimeException> exception,
      String errorDescription) {
    assertThatExceptionOfType(exception)
        .isThrownBy(() -> aGivenReceipt().with(wrongData).build())
        .withMessage(errorDescription);
  }

  private RecipeBuilder aGivenReceipt() {
    return RecipeBuilder.getFor("aVendor").with(aBaunchOfData);
  }
}
