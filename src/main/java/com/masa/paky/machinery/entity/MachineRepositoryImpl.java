...
public class MachineRepositoryImpl implements MachineryRepository{
    ...
    public Optional<Machinery> findById(String id) {
        return ofNullable(
          entityManager.find(Machinery.class,id)
        );
    }
    ...
    private TypedQuery<Recipe> getRecipeTypedQuery(String machineryId) {
        String sql = "select r from Recipe as r where r.recipeId = (select m.recipeId from Machinery as m where m.machineryId = :machineryId)";
