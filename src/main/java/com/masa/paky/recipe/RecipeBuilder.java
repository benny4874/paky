...
public class RecipeBuilder {
    ...
    public RecipeBuilder with(
            Consumer<RecipeBuilder> builder
    ) {
        builder.accept(this);
        return this;
    }
    ...
    private void checkExpiration() {
        ...
        if ((new Date()).after(expiration)) throw new NoExpirationDateException("Expiration date is invalid");
    }

    private void checkQuantity() {
        if (quantity == 0) throw new InvalidQuantityException("Quantity is invalid (must be greater than 0)");
        if (quantity < 0) throw new InvalidQuantityException("Quantity is invalid (must not be negative)");
    }
