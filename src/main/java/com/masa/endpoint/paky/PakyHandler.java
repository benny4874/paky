...
public class PakyHandler {
    ...
    HttpResponse<Paky> getPaky(@PathVariable(value = "pakyId") String pakyId) {
        ...
        if (paky.isPresent())
            return HttpResponse.ok(paky.get());
        else
    ...
    HttpResponse<PakyAnswer> bookPaky(
            ...
            @Body ExpeditionCommand bookRequest) {

        try {
            PakyReservationManager reservationManager = new PakyReservationManager(pakyRepository, vendorRepository);
    ...
    HttpResponse<PakyAnswer> sendPaky(
            @PathVariable(value = "pakyId") String pakyId,
            @Body ExpeditionCommand recipient) {
        try {
            PakyExpeditionManager<Vendor, String> pakyExpeditionManager = new PakyExpeditionManager<>(vendorRepository, pakyRepository);
